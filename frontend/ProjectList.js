...
function ProjectList({ token }) {
  ...
  useEffect(() => {
    const fetchProjects = async () => {
      try {
        const response = await fetch('https://codebloom-finalreleasse.onrender.com/projects', {
          headers: {
  ...
  const confirmDelete = async () => {
    try {
      const response = await fetch(`https://codebloom-finalreleasse.onrender.com/projects/${selectedProjectId}`, {
        method: 'DELETE',
        headers: {
          Authorization: `Bearer ${token}`
        }
